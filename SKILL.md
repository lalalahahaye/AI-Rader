...
## Reddit：403 与可选 OAuth

- **默认（读者仍零 key）**：构建脚本使用 **Reddit 合规 User-Agent**、**按子版块串行**请求（`reddit.requestDelayMs`）、**403/429 退避重试**（`reddit.maxRetries`），失败时回退 **`hot.rss`** 解析。
- **若 CI 仍大量 403**：在仓库 Secrets 配置 **`REDDIT_CLIENT_ID`** + **`REDDIT_CLIENT_SECRET`**（reddit.com 上创建「script」类应用，**application-only** / `client_credentials`）。Token 仅用于构建机，**不会**写入 `feed-investor.json`。
...
5. **Reddit empty / 403**: builder uses **compliant User-Agent**, **serial sub fetches** + **`hot.rss` fallback**; if CI still blocks, set Secrets **`REDDIT_CLIENT_ID`** + **`REDDIT_CLIENT_SECRET`**. Older empty buckets were also from **`includeKeywords`** — defaults use **`skipThesisFilter: true`** on Reddit (exclude-only).
